...
void linkModelSplinesToGUI() {
	...
	if(model.isVolumetricModel()) {
		...
		for(uint i=0; i<surfaces[0].size(); i++) {
			RectDomain pSpan = surfaces[0][i]->parameterDomain();
			// add edges
			vector<shared_ptr<SplineCurve> > c;
			c.push_back(shared_ptr<SplineCurve>(surfaces[0][i]->constParamCurve(pSpan.vmin(), true)) );
			c.push_back(shared_ptr<SplineCurve>(surfaces[0][i]->constParamCurve(pSpan.vmax(), true)) );
			c.push_back(shared_ptr<SplineCurve>(surfaces[0][i]->constParamCurve(pSpan.umin(), false)) );
			c.push_back(shared_ptr<SplineCurve>(surfaces[0][i]->constParamCurve(pSpan.umax(), false)) );
			curves.push_back(c);
