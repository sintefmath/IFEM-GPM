...
int main(int argc, char** argv)
{
  ...
  if (surface_model) {
    ...
    for (size_t i = 0;i < in_surf.size();i++) {
      ...
      std::vector<double>::const_iterator first =  ++s_it->basis(0).begin();
      std::vector<double>::const_iterator last  =  --s_it->basis(0).end();
      Go::BsplineBasis b1 = Go::BsplineBasis(s_it->order_u()-1,first,last);
      first =  ++s_it->basis(1).begin();
      last  =  --s_it->basis(1).end();
      Go::BsplineBasis b2 = Go::BsplineBasis(s_it->order_v()-1,first,last);
